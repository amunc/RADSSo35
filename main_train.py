...
'''BEGIN of Mandatory execution parameters - IT IS SUGGESTED NOT TO MODIFY.
It can cause impredictable behaviour'''
...
if os.path.exists(path_to_configuration_file):
    config_parser_file = open(path_to_configuration_file, encoding=glod.get_encoding())
...
output_data_section = glod.get_output_section_name()
testing_section = glod.get_validation_section_name()

'''Reading field separator for the input and ouput files, the format of the images
and the execution mode'''
show_debug = False
char_encoding = glod.get_encoding() #encoding used along the script
lc_output_format = glod.get_output_img_files_extension() #learning curves output format
pkl_extension = glod.get_output_pkl_files_extension() #models output formar
compute_lc = eval(config_parser.get(input_data_section, glod.get_compute_lc_name()))
input_files_extension = glod.get_input_files_extension()
selected_models = False

list_of_not_valid_characters = ['\\', '/', ':', '*', '?', '<', '>', '|']
max_lenght_output_names = 180 #in linux systems it can be incremented
list_of_parameters_models_events_dict = [glod.get_learning_key(),
                                         glod.get_model_path_key(),
                                         glod.get_reasignment_dict_key(),
                                         glod.get_current_features_key(),
                                         glod.get_original_features_key()]
list_names_metrics = [glod.get_precision_key(),
                      glod.get_recall_key(),
                      glod.get_specificity_key(),
                      glod.get_f1_score_key(),
                      glod.get_mcc_key()]
target = 'generic' #to create generic error log if error in objective input target
confusion_matrix_delimiter = ','
max_pars = [30, 30]
penalize_falses = eval(config_parser.get(input_data_section, glod.get_penalization_name()))

'''END of Mandatory execution parameters - IT IS SUGGESTED NOT TO MODIFY.
It can cause impredictable behaviour'''
...
default_matrix = eval(config_parser.get(input_data_section, glod.get_default_matrix_name()))
validation_mode = eval(config_parser.get(testing_section, glod.get_validation_mode_name()))
...
try:
    ...
    errors_file = os.path.join(log_path, config_parser.get(logs_section, glod.get_log_error_name())+target+'.' + glod.get_log_files_extension())
    ...
    exponent = int(config_parser.get(input_data_section, glod.get_exponent_number()))

    n_feat = float(config_parser.get(input_data_section, glod.get_percentage_features()))


    '''Setp 0.2: reading name feature (feature with events names)'''
    name_variable_with_events = config_parser.get(input_data_section, glod.get_event_name_feature_name())

    target = auxf.check_input_target(sys.argv)

    label_non_catalogued = int(config_parser.get(input_data_section, glod.get_non_catalogued_label_name()))

    errors_file = os.path.join(log_path, config_parser.get(logs_section, glod.get_log_error_name()) + target+'.' + glod.get_log_files_extension())
    repg.register_log([errors_file], "Errors registered:\n", 0, char_encoding)
    log_file = os.path.join(log_path, config_parser.get(logs_section, glod.get_log_execution_name()) + target+'.' + glod.get_log_files_extension())
    time_log = os.path.join(log_path, config_parser.get(logs_section, glod.get_log_time_execution_name()) + target+'.' + glod.get_log_files_extension())
    ...
    path_to_input_files = os.path.join(BASE_PATH, config_parser.get(input_data_section, glod.get_path_to_root_directory_input_files_name()))
    path_to_dir_user_files = os.path.join(BASE_PATH, config_parser.get(input_data_section, glod.get_user_files_directoryname()))
    ...
    maximum_number_input_files = int(config_parser.get(input_data_section, glod.get_maximum_number_files_to_read_name()))
    ...
    maximum_number_of_observations_to_process = int(config_parser.get(input_data_section, glod.get_maximum_number_observations_to_read_name()))
    ...
    events_filename = config_parser.get(input_data_section, glod.get_events_filename_name())
    ...
    discarded_variables_filename = config_parser.get(input_data_section, glod.get_user_discarded_variables_filename_name())
    ...
    data_division_percentaje = float(config_parser.get(input_data_section, glod.get_train_test_division_percentaje_name()))
    try:
    ...
    list_common_discarded_variables = auxf.load_dictionary_of_features(path_to_discarded_variables_file, glod.get_user_discarded_common_features())
    substep_finish_time = datetime.datetime.fromtimestamp(time.time())
    ...
    list_of_supervised_models_av = list(supervised_models_dictionary.values())
    list_of_supervised_models_av.remove(glod.get_all_learning_modes_name())

    unsupervised_models_dictionary = datr.generate_dictionary_available_unsupervised_learning_models()
    list_of_unsupervised_models_av = list(unsupervised_models_dictionary.values())
    list_of_unsupervised_models_av.remove(glod.get_all_learning_modes_name())

    if selected_models:#Se ha introducido para poder seleccionar n modelos
        list_indentifiers_models = ['T', 'A', 'G', 'M', 'R', 'K']
        customized_supervised_models_dictionary = datr.generate_dictionary_customized_supervised_learning_models(list_indentifiers_models)
        customized_unsupervised_models_dictionary = datr.generate_dictionary_customized_unsupervised_learning_models(list_indentifiers_models)
        full_list_of_available_models = [list(customized_supervised_models_dictionary.values()), list(customized_unsupervised_models_dictionary.values())]
    else:
    ...
    for event in list_of_events:
        list_available_learnings.append(glod.get_all_learning_modes_name())
    ...
    '''Obtaining percentil'''
    percentil = datr.checking_percentil_value(config_parser.get(input_data_section, glod.get_percentage_relevant_variables()))

    ''' Stablishing output directories '''
    ruta_directorio_raiz_salida = os.path.join(BASE_PATH, config_parser.get(output_data_section, glod.get_output_directory_rootname()))
    ...
    report_data = repg.create_basic_report_data_dict([percentil, target, main_metric, feature_selection_method, penalize_falses], list_common_discarded_variables, "'" + os.path.join(path_to_dir_auxiliary_files, glod.get_logo_name()) + "'")
    repg.create_report_current_execution(report_data, [list_of_events, list_of_user_relevant_variables_event, list_of_discarded_variables_events, list_available_learnings, list_of_mod], available_learnings_dict, auxiliary_directory_filename, ruta_directorio_raiz_salida)
    ...
    path_to_directory_prediction_models = os.path.join(BASE_PATH, config_parser.get(output_data_section, glod.get_output_directory_name_prediction_models()))
    auxf.create_directory(path_to_directory_prediction_models)
    prediction_models_for_events_dictionary, handler_prediction_models_for_events_dictionary = auxf.retrieve_dictionary_and_handler(os.path.join(path_to_directory_prediction_models, config_parser.get(output_data_section, glod.get_prediction_models_dictionary_filename_name())))
    ...
    '''  Beginning of events processing '''
    repg.register_log([time_log], '\n###############################################\n############   Processing events   ############\n###############################################\n\n', '', char_encoding)
    print('Available problems in the dataset: ', list_of_events, "\n")
    for event_index in range(len(list_of_events)):
        ...
        path_to_reports_current_event_directory = os.path.join(path_to_current_event_directory, config_parser.get(output_data_section, glod.get_output_directory_name_report_name()))
        ...
        '''Substep 2.3: Creating individual log for the current event'''
        substep_init_time = datetime.datetime.fromtimestamp(time.time())
        repg.register_log([log_file], '>>>>>>Substep 2.3: Creating individual log for '+ current_event  +" " + substep_init_time.strftime('%Y-%m-%d %H:%M:%S') + "\n", '', char_encoding)

        current_event_individual_log = "Log_" + current_event + "." + glod.get_log_files_extension()
        ...
        try:
            ...
            list_discarded_variables_current_event += list_of_discarded_variables_events[event_index]
            ...
            report_data = repg.create_report_data_dict(current_event, percentil, target, list_discarded_variables_current_event, "'" + os.path.join(path_to_dir_auxiliary_files, glod.get_logo_name()) + "'")
            ...
            df_catalogued_data, df_datos_no_catalogados = auxf.split_dataset_catalogued_non_catalogued(vector_full_paths_input_files, name_variable_with_events, current_event, target, maximum_number_of_observations_to_process, label_non_catalogued, input_files_delimiter, [log_file], list_discarded_variables_current_event, char_encoding)

            existen_datos_catalogados = not df_catalogued_data.empty
            catalogar_no_validos = list(df_datos_no_catalogados.columns) == list(df_catalogued_data.columns)
            ...
            if not existen_datos_catalogados:
                ...
                report_data = repg.update_report_warning_info(report_data, current_event + ' has not valid data. Models for this event could not be calculated because catalogued data was not found. ')
                ...
                '''Replace NaN values'''
                for column_to_correct in df_catalogued_data.columns:
                    if column_to_correct != target:
                        df_catalogued_data[column_to_correct] = df_catalogued_data[column_to_correct].replace(np.nan, 0)

                if not df_datos_no_catalogados.empty:
                    df_datos_no_catalogados[observation_number] = range(len(df_catalogued_data)+1, len(df_catalogued_data)+1+len(df_datos_no_catalogados))
                ...
                if solved_problem:#only one target
                    step_finish_time = datetime.datetime.fromtimestamp(time.time())
                    ...
                    report_data = repg.update_report_warning_info(report_data, current_event + ' has only one target. The classification problem is already solved ')
                    repg.create_report_current_model(report_data, [], auxiliary_directory_filename, path_to_reports_current_event_directory, char_encoding)
                    raise FollowFlowException
                    ...
                    if not validation_mode:
                        ...
                        df_catalogued_data = df_catalogued_data.sort_values(by=[observation_number])
                        df_catalogued_data.to_csv(os.path.join(path_to_directory_event_data, 'numbered_' + current_event + '_data_'+ target +'.'+ input_files_extension), index=False, sep=input_files_delimiter, encoding=char_encoding)

                    else:
                        '''Splitting data in Training-Test and Validation datasets if validation_mode is True'''
                        validation_division_percentaje = float(config_parser.get(testing_section, glod.get_validation_division_percentage_name()))
                        ...
                        nombre_fichero_datos_validacion = os.path.join(path_to_directory_event_data, target+'validation_data_'+current_event+'.'+input_files_extension)
                    ...
                    if not df_datos_no_catalogados.empty:
                        '''Saving non-catalogued data for a possible prediction phase'''
                        df_datos_no_catalogados.to_csv(os.path.join(path_to_directory_event_data, 'unknown_' + current_event + '_data_'+ target +'.'+ input_files_extension), index=False, sep=input_files_delimiter, encoding=char_encoding)
                        shutil.move(os.path.join(path_to_directory_event_data, 'unknown_' + current_event + '_data_'+ target +'.'+ input_files_extension), os.path.join(ruta_directorio_datos_validacion, 'unknown_' + current_event + '_data_'+ target +'.'+ input_files_extension))
                        repg.register_log(array_of_paths_log_files, '>>>>>> Substep 3.2.1 Saving non-catalogued data in a file for ' + current_event + ' ' + substep_init_time.strftime('%Y-%m-%d %H:%M:%S') + "\n", '', char_encoding)
                    ...
                    if not existence_possible_features_train_models:
                        ...
                        useless_features = [observation_number] #obsnumber feature is discarded to apply mutual information function
                        lista_features_scores = []
                        if feature_selection_method == 'fisher':
                            path_to_fisher_output_directory = os.path.join(path_to_current_event_directory, config_parser.get(output_data_section, glod.get_output_directory_name_fisher_name()))
                            auxf.create_directory(path_to_fisher_output_directory)
                            lista_features_scores = metr.apply_fisher_score_to_current_features_using_percentil(current_event, target, percentil, path_to_fisher_output_directory, df_catalogued_data, list_user_relevant_variables_actual_event, useless_features, log_file, char_encoding)

                        else:
                            path_to_mif_output_directory = os.path.join(path_to_current_event_directory, config_parser.get(output_data_section, glod.get_output_directory_name_mif_name()))
                        ...
                        features_used_during_process, dic_relevantfeat_scores = auxf.get_features_to_train_models(lista_features_scores, features_discarded_mif_process, n_feat)
                        ...
                        if features_used_during_process == []: #process ends if no relevant variables were found
                            ...
                            report_data = repg.update_report_warning_info(report_data, current_event + ' has not relevant features (all scores ar under computed percentil value). The event is excluded')
                            repg.create_report_current_model(report_data, [], auxiliary_directory_filename, path_to_reports_current_event_directory, char_encoding)
                            raise FollowFlowException
                            ...
                            '''Beginning the congigurations of the list of models or the current event'''
                            list_available_supervised_models = list(supervised_models_dictionary.values())
                            list_non_supervised_available_models = list(unsupervised_models_dictionary.values())

                            list_available_supervised_models.remove(glod.get_all_learning_modes_name())
                            list_non_supervised_available_models.remove(glod.get_all_learning_modes_name())
                            ...
                            if default_matrix:#user specified
                                diagonal_value = config_parser.get(input_data_section, glod.get_diagonal_weight_name())
                                w = auxf.compute_matrix_weights(len(target_recodified_values), diagonal_value)
                            else:
                                w = ast.literal_eval(config_parser.get(input_data_section, glod.get_matrix_of_weights_fp_fn_name()))
                                try:
                            ...
                            if show_debug:
                                auxf.print_debug_info(compute_lc, penalize_falses, default_matrix, validation_mode, exponent, main_metric, feature_selection_method, n_feat, percentil, name_variable_with_events, target, label_non_catalogued, w)
                            if list_available_learnings_current_event != available_learnings_dict[2]:# if supervised or both
                                ...
                                for supervised_model_name in list_supervised_models_current_event:
                                    ...
                                    try:
                                        ...
                                        report_data = repg.update_report_model_time(report_data, supervised_model_name, glod.get_time_model_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, supervised_model_name, glod.get_time_sel_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, supervised_model_name, glod.get_time_sel_finish_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, supervised_model_name, glod.get_time_train_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, supervised_model_name, glod.get_time_train_finish_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        accuracy_current_model = supe.calculate_model_accuracy(current_trained_model, df_test, features_used_train_models, target, 0)
                                        ...
                                        if compute_lc:
                                            ...
                                            for indice_sc in range(len(lista_scorings)):
                                                ...
                                                for index_cv_it in range(len(cv_iterators)):
                                                    ...
                                                    name_of_the_curve = "Learning curve for It_" + str(cv_iterator_name) + "_Sc_"+ score_name + "_" + supervised_model_name + "." + lc_output_format
                                        ...
                                        confusion_matrix_name_train = 'confusion_matrix'+ supervised_model_name +'_'+ str.lower(glod.get_train_option_key())
                                        ...
                                        metr.confusion_matrix_to_csv(matrix_summary_clasifications_train, list_targets_current_event, os.path.join(path_to_supervised_model_directory, glod.get_confusion_matrix_incorrect_classifications_train_name() + "." + 'csv'))
                                        ...
                                        confusion_matrix_name_test = 'confusion_matrix_'+supervised_model_name+ '_' + str.lower(glod.get_test_option_key())
                                        ...
                                        metr.confusion_matrix_to_csv(matrix_summary_clasifications_test, list_targets_current_event, os.path.join(path_to_supervised_model_directory, glod.get_confusion_matrix_incorrect_classifications_test() + "." + 'csv'))
                                        ...
                                        report_data = repg.update_model_feature(report_data, supervised_model_name, glod.get_confussion_matrix_train_path_key(), os.path.join(path_to_supervised_model_directory, confusion_matrix_name_train + "." + lc_output_format))
                                        ...
                                        report_data = repg.update_model_feature(report_data, supervised_model_name, glod.get_confussion_matrix_test_path_key(), os.path.join(path_to_supervised_model_directory, confusion_matrix_name_test + "." + lc_output_format))
                                        ...
                                        try:
                                            if main_metric == glod.get_mcc_metric_key():
                                                if compute_lc:
                                                    decision_index_current_model = auxf.weight_for_decision_function(train_sizes, train_scores_mean, test_scores_mean, confusion_matrix_model_test, w, mcc_micro_avg_test, compute_lc, penalize_falses=penalize_falses)
                                                else:
                                                    decision_index_current_model = auxf.weight_for_decision_function([], [], [], confusion_matrix_model_test, w, mcc_micro_avg_test, compute_lc, penalize_falses=penalize_falses)
                                            else:
                                                if compute_lc:
                                                    decision_index_current_model = auxf.weight_for_decision_function(train_sizes, train_scores_mean, test_scores_mean, confusion_matrix_model_test, w, accuracy_current_model, compute_lc, penalize_falses=penalize_falses)
                                                else:
                                                    decision_index_current_model = auxf.weight_for_decision_function([], [], [], confusion_matrix_model_test, w, accuracy_current_model, compute_lc, penalize_falses=penalize_falses)
                                        ...
                                        path_to_store_current_generated_model = supe.save_current_model_to_file(current_trained_model, path_to_supervised_model_directory, supervised_model_name +'.' + pkl_extension)
                                        ...
                                        repg.register_log([errors_file], '\n>>'+supervised_model_name +' model was not created for the event '+ current_event + ' because ' + str(excep) + ' ' + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') + "\n\n", '', char_encoding)
                            ...
                            if list_available_learnings_current_event != available_learnings_dict[1]:# if unsupervised or both
                                ...
                                for unsupervised_model_name in list_unsupervised_models_current_event:
                                    ...
                                    try:
                                        ...
                                        report_data = repg.update_report_model_time(report_data, unsupervised_model_name, glod.get_time_model_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, unsupervised_model_name, glod.get_time_sel_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, unsupervised_model_name, glod.get_time_sel_finish_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, unsupervised_model_name, glod.get_time_train_init_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        report_data = repg.update_report_model_time(report_data, unsupervised_model_name, glod.get_time_train_finish_key(), datetime.datetime.fromtimestamp(time.time()))
                                        ...
                                        #1 - Get dictionary that associates original target value to cluster value {target:cluster}
                                        dictionary_of_reasignation_of_targets_inverse = {}
                                        for generated_cluster in diccionario_reasignacion:
                                            target_original_asignado = diccionario_reasignacion[generated_cluster]
                                            dictionary_of_reasignation_of_targets_inverse[target_original_asignado] = generated_cluster

                                        #2- The original target value is recodified with the corresponding cluster value based on the previous dictionary
                                        ...
                                        if compute_lc:
                                            unsup_model_initialized = unsu.initialize_model(unsupervised_model_name, parameters_current_model, unsupervised_models_dictionary)
                                            lista_scorings = metr.get_scoring_methods_unsupervised()
                                            cv_iterators = metr.get_all_available_cross_validation_iterators(10, 0.2)
                                            for indice_sc in range(len(lista_scorings)):
                                                score_curve = lista_scorings[indice_sc]
                                                score_name = score_curve[0]
                                                ...
                                                for index_cv_it in range(len(cv_iterators)):
                                                    ...
                                                    cv_it = cv_iterators[index_cv_it][1]
                                                    train_sizes_def = np.linspace(.1, 1.0, 10)
                                                    train_sizes, train_scores, test_scores = metr.compute_learning_curve(unsup_model_initialized, df_catalogued_data[features_used_train_models], train_true_learning_curve, score_curve, cv_it, train_sizes=train_sizes_def)

                                                    name_of_the_curve = "Learning curve for It_" + str(cv_iterator_name) + "_Sc_"+ score_name + "_" + unsupervised_model_name + "." + lc_output_format
                                                    full_path_to_learning_curve_file = os.path.join(path_to_current_unsupervised_model_output_directory, name_of_the_curve)
                                                    train_scores_mean, test_scores_mean = metr.save_learning_curve(train_sizes, train_scores, test_scores, "Learning curve(It_" +str(cv_iterator_name) + "_Sc_" +score_name +") for "+ unsupervised_model_name, full_path_to_learning_curve_file, lc_output_format)
                                        ...
                                        '''Subsubstep 4.3.1.5 Getting confusion matrices and decision index for the current unsupervised model'''
                                        subsubstep_init_time = datetime.datetime.fromtimestamp(time.time())
                                        ...
                                        confusion_matrix_name_train = 'confusion_matrix_'+ unsupervised_model_name
                                        ...
                                        metr.confusion_matrix_to_csv(matrix_summary_clasifications, list_targets_current_event, os.path.join(path_to_current_unsupervised_model_output_directory, glod.get_confusion_matrix_incorrect_classifications_unsu_name() + "." + input_files_extension))

                                        #ts=train_sizes,ss=sample_scores,cvs=cv_scores,cm=confusion_matrix,w=weights,ac=accuracy
                                        try:
                                            if main_metric == glod.get_mcc_metric_key():
                                                if compute_lc:
                                                    decision_index_current_model = auxf.weight_for_decision_function(train_sizes, train_scores_mean, test_scores_mean, confusion_matrix_model_train, w, mcc_micro_avg_train, compute_lc, penalize_falses=penalize_falses)
                                                else:
                                                    decision_index_current_model = auxf.weight_for_decision_function([], [], [], confusion_matrix_model_train, w, mcc_micro_avg_train, compute_lc, penalize_falses=penalize_falses)
                                            else:
                                                if compute_lc:
                                                    decision_index_current_model = auxf.weight_for_decision_function(train_sizes, train_scores_mean, test_scores_mean, confusion_matrix_model_train, w, accuracy_current_model, compute_lc, penalize_falses=penalize_falses)
                                                else:
                                                    decision_index_current_model = auxf.weight_for_decision_function([], [], [], confusion_matrix_model_train, w, accuracy_current_model, compute_lc, penalize_falses=penalize_falses)                                            
                                        except Exception:                                            
                                        ...
                                        report_data = repg.update_model_feature(report_data, unsupervised_model_name, glod.get_confussion_matrix_train_path_key(), os.path.join(path_to_current_unsupervised_model_output_directory, confusion_matrix_name_train + "." + lc_output_format))
                                        ...
                                        path_to_store_current_generated_model = unsu.save_model_to_disk(current_trained_model, path_to_current_unsupervised_model_output_directory, unsupervised_model_name +'.' + pkl_extension)
                                        ...
                                        repg.register_log([errors_file], '\n>>'+unsupervised_model_name +' model was not created for the event '+ current_event + ' because ' + str(excep) + "\n", '', char_encoding)
                                        repg.register_log([errors_file], '>> An error has happened '+ str(excep) + "\n", '', char_encoding)
                                        repg.register_log(array_of_paths_log_files, '>> An error has happened check error log #######' + "\n", '', char_encoding)
                                        error_trace = "\n" + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ "\n Full trace:\n" + str(traceback.format_exc())
                                        repg.register_log([errors_file], error_trace, '', char_encoding)
            ...
            best_model_characteristics = dictionary_of_selected_models_for_events[best_model[0]]

            '''Copying the model to the destiny directory'''
            best_model_final_pkl_name = config_parser.get(output_data_section, glod.get_output_directory_name_prediction_models()) + '/' + current_event + "_" + target +"_"+best_model[0]+ "." + pkl_extension


            pred_path = config_parser.get(output_data_section, glod.get_output_directory_name_prediction_models())
            for root_pred, directory_pred, files_pred in os.walk(pred_path):
                for filename_pred in files_pred:
                    if(target in filename_pred) and(current_event in filename_pred) and('.' + pkl_extension in filename_pred):
                        try:
            ...
            prediction_models_for_events_dictionary = auxf.update_prediction_models_original_features_for_event_in_prediction_dictionary(prediction_models_for_events_dictionary, current_event, target, best_model_characteristics, list_of_parameters_models_events_dict)
            handler_prediction_models_for_events_dictionary.close()
            try:
                '''Saving in memory actual state of dictionary'''
                path_to_prediction_models_dictionary = os.path.join(path_to_directory_prediction_models, config_parser.get(output_data_section, 'prediction_models_dictionary_filename'))
                ...
                estado_inicial_diccionario = copy.deepcopy(diccionario_modelos)
                handler.close()
                ...
                repg.create_report_current_dictionary_models(prediction_models_for_events_dictionary, [auxiliary_directory_filename, ruta_directorio_raiz_salida], list_of_parameters_models_events_dict, "'" + os.path.join(path_to_dir_auxiliary_files, glod.get_logo_name()) + "'", char_encoding)
                handler.close()
            ...
            repg.register_log(array_of_paths_log_files, '>> An error has happened check error log #######' + "\n", '', char_encoding)
            error_trace = "\n" + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ "\n Full trace:\n" + str(traceback.format_exc())
            repg.register_log([errors_file], error_trace + "\n", '', char_encoding)
            report_data = repg.update_report_warning_info(report_data, str(excep) + ' for ' + current_event + '.')
            repg.create_report_current_model(report_data, [], auxiliary_directory_filename, path_to_reports_current_event_directory, char_encoding)
    ...
    print("total time: " + str(execution_finish_time - execution_init_time))
    ...
    print('Training process finished. Check the logs and reports for more information.\n Thanks for training using RADSSo')


except KeyError as excep:
    repg.register_log([errors_file], '>> Execution aborted by the user '+ str(excep) + ' ' + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ '\n', '', char_encoding)
    error_trace = "Full trace:\n" + str(traceback.format_exc())
    repg.register_log([errors_file], error_trace, '', char_encoding)
    ...
    repg.register_log([errors_file], '>> An IndexError exception has happened '+ str(excep) + ' ' + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ '\n', '', char_encoding)
    error_trace = "Full trace:\n" + str(traceback.format_exc())
    repg.register_log([errors_file], error_trace, '', char_encoding)

except ValueError as excep:
    repg.register_log([errors_file], '>> A ValueError exception has happened '+ str(excep) + ' ' + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ '\n', '', char_encoding)
    error_trace = "Full trace:\n" + str(traceback.format_exc())
    repg.register_log([errors_file], error_trace, '', char_encoding)

except IOError as excep:
    repg.register_log([errors_file], '>> An IOError has happened '+ str(excep) + ' ' + datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')+ u'\n', '', char_encoding)
    error_trace = "Full trace:\n" + str(traceback.format_exc())
    repg.register_log([errors_file], error_trace, '', char_encoding)

except Exception as excep:
    repg.register_log([errors_file], '>> An Unknow Exception has happened '+ str(excep) + ' ' +datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') + '\n', '', char_encoding)
    error_trace = "Full trace:\n" + str(traceback.format_exc())
    repg.register_log([errors_file], error_trace, '', char_encoding)
