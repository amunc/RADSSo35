...
def get_items_dict(list_of_available_items):
    '''
    This function allows to generate the dictionary of elements for the user to be able to select them
    ...
    list_of_items_in_alphabetic_order.append(glod.get_all_learning_modes_name())
    list_of_items_in_alphabetic_order.reverse()    
    ...
    dictionary_numeric_options_linked_to_items[-1] = glod.get_exit_continue_name()
...
def show_elegible_items (items_dictionary,topic,enco):
    '''
    This function allows to show to the user and specific menu of options for the current topic for
    ...
    for option in items_dictionary:
        '''if(enco != glod.get_none_encoding()):            
            ...
            for element in items_dictionary[option]:
                corrected +=  str(chr(ord(element)).encode(enco))
            shown_string+=corrected + "\n"
        else:'''
        shown_string+= str(option) + " - " + items_dictionary[option] + "\n"
...
def show_format_selected_items(list_of_numbers_related_to_items,items_dictionary,topic):
    '''
    This function allows to display to the user the items that he has selected prevously realted to a topic. As the user
    ...
    :param dictionary<int:str> items_dictionary: Dictionary with the available options for the current topic
    :str topic: area to which the displayed options belongs to
...
'''
    ...
    It allows to recognize all the existing features in the specififed input file in an automatic way.
...
def get_events_and_variables_from_user_interaction(vector_with_fullpaths_of_input_files,feature_events_name,input_files_delimiter,list_of_not_valid_characters,enco):
    '''
    ...
    :param feature_events_name: Feature with the values from which to obtain the names of the events
    :param str input_files_delimiter: Delimiter of the fields for the input files
    :param list<str> list_of_not_valid_characters: List with characters not valid be contained by an event name
    :param str enco: String with the encoding to be used
    
    
    :return: dictionary_events_features: Dictionary with the lists of features selected as important and discarded for each event    
    :rtype: dictionart<str:dictionaryzstr:list>>
    '''   
                                                                                               
    elegible_events,elegible_features = recognize_available_events_and_variables_automatically(vector_with_fullpaths_of_input_files,feature_events_name,input_files_delimiter,list_of_not_valid_characters,enco)
    dictionary_with_elegible_events = get_items_dict(elegible_events)
    ...
    reset_process = ''
    topic = 'events'
    while(reset_process != 'N'):
        list_of_selected_events_by_the_user = []
        selected_option =''
        while(selected_option != -1 and selected_option !=0):
            sys.stdout.flush() #Avoid content to be stored in temporary memory
            try:                
                print (show_elegible_items(dictionary_with_elegible_events,topic,enco))
                usr_selection = input()
                ...
                pass
            sys.stdout.flush() 
            if (selected_option in dictionary_with_elegible_events and selected_option !=-1 and selected_option not in list_of_selected_events_by_the_user):
                list_of_selected_events_by_the_user.append(selected_option)
            elif(selected_option != -1):
                print ("Incorrect event number")
                sys.stdout.flush() 
                selected_option = ''
            
        #>At this point list of events is already configured
        numeric_list_selected_events = []
        if(list_of_selected_events_by_the_user != []):
            list_of_selected_events_by_the_user,numeric_list_selected_events = show_format_selected_items(list_of_selected_events_by_the_user,dictionary_with_elegible_events,topic)
            user_confirmation = ''
            while(user_confirmation !='Y' and user_confirmation!='N'):
                sys.stdout.flush() 
                try:
                    user_confirmation = input("Are the selected events correct?['Y','N']:")
            ...
            raise FinishFlowException


    print (">>>> Selecting events, process completed <<<<\n")
    sys.stdout.flush()
    
    '''        
    Features selection process    
    >>>If execution continues by here is because at least one event was selected
    '''
    ...
    while (event_selected_index < len(list_of_selected_events_by_the_user)):
        ...
        corrected_event = current_selected_event
        '''for elemento in current_selected_event:
            corrected_event +=  chr(ord(elemento)).encode(enco)'''
        ...
        while(reset_features_process != 'N'):
            ...
            if(user_confirmation == 'Y'):
                ...
                dictionary_events_features[current_selected_event][glod.get_relevant_features_input_name()] = list_of_selected_features#corrected_relevant_features
                if(list_of_selected_features != []):
    ...
    while (event_selected_index < len(list_of_selected_events_by_the_user)):
        ...
        while(reset_features_process != 'N'):
            ...
            #>Tenemos la lista con los eventos
            numeric_list_selected_features = []
            if(list_of_selected_features != []):
                list_of_selected_features,numeric_list_selected_features = show_format_selected_items(list_of_selected_features,dictionary_with_elegible_features,topic)
                sys.stdout.flush() 
            ...
            if(user_confirmation == 'Y'):
                ...
                for d_feature in list_of_selected_features:
                    corrected_feature = d_feature#''
                    '''for character in d_feature:
                        corrected_feature+= chr(ord(character)).encode(enco)'''
                    corrected_discarded_features.append(corrected_feature)                               
                dictionary_events_features[current_selected_event][glod.get_discarded_features_input_name()] = list_of_selected_features#corrected_discarded_features
                if(list_of_selected_features != []):
                    print ("Important features typed by the user for event: " + corrected_event +"\n ")
                    sys.stdout.flush()
                    for elemento in list_of_selected_features:                    
    ...
    print (">>>>> Important features stored successfully <<<<<<<")
    sys.stdout.flush()
...
def get_list_events_features_json_file(fullpath_to_events_json_file,list_of_not_valid_characters):
    ...
    print ('Reading json with specific variables (' + glod.get_semi_automated_mode_name() + ' mode)')
    ...
    if(os.path.lexists(fullpath_to_events_json_file)):
        json_events_features = auxf.load_json_file(fullpath_to_events_json_file)
        for event in json_events_features:
            ...
            if(add_event):
                ...
                try:
                    list_current_relevant_features = json_events_features[event][glod.get_relevant_features_input_name()]                    
                ...
                try:
                    list_current_dicarded_variables = json_events_features[event][glod.get_discarded_features_input_name()]
...
def generate_learnings_dict():
    ...
    dictionary_learnings[0] =  glod.get_all_learning_modes_name()
    dictionary_learnings[1] =  glod.get_supervised_name()
    dictionary_learnings[2] =  glod.get_unsupervised_name() 
...
def get_dictionary_with_learnings_related_to_models():
    ...
    for key in supervised_models_dictionary:
        ...
        if(accumulated_supervised_models != glod.get_all_learning_modes_name()):
    ...
    for key in unsupervised_models_dictionary:
        ...
        if(accumulated_unsupervised_models != glod.get_all_learning_modes_name()):
...
def generate_dictionary_available_supervised_learning_models():
    ...
    dictionary_supervised_models[0] = glod.get_all_learning_modes_name()    
    dictionary_supervised_models[1] = glod.get_tree_model_name()
    dictionary_supervised_models[2] = glod.get_ada_model_name()
    dictionary_supervised_models[3] = glod.get_boosting_model_name()                                   
    dictionary_supervised_models[4] = glod.get_random_forest_model_name()
    dictionary_supervised_models[5] = glod.get_mlp_model_name() 
...
def generate_dictionary_available_unsupervised_learning_models():
    ...
    dictionary_unsupervised_models[0] = glod.get_all_learning_modes_name()
    dictionary_unsupervised_models[1] = glod.get_kmeans_model_name()
    
    return dictionary_unsupervised_models


def generate_dictionary_customized_supervised_learning_models(list_models):
    '''
    It allows to get the dictionary with the available supervised learning models
    The list should contain the initial letters of the models: ['T','A','G','R','M']
    T - Tree
    A - Ada
    G - Gradient Boosting
    R - Random Forest
...
def generate_dictionary_customized_unsupervised_learning_models(list_models):
    '''
    It allows to get the dictionary with the available unsupervised learning models
    The list should contain the initial letters of the models: ['K']
    K - Kmeans
    ...
    for elemento in list_models:
        if(elemento == 'K'):
            dictionary_unsupervised_models[0] = glod.get_kmeans_model_name()
...
def check_correspondency_input_lists_events_learnings_and_models(list_of_events,list_of_learnings,list_of_models):
    ...
    :exception ValueError: If any of the models do not correspond to the learnings
    :exception ValueError: If any of the models do not exist
    :exception IndexError: If the lengths events do not match with the number of models or the number of learnings
    ...
    if((lenght_events == lenght_learnings)and(lenght_learnings == length_models)):
        ...
        for indice in range(len(list_of_learnings)):
            ...
            if(aprendizaje == glod.get_all_learning_modes_name()):
                modelos_pedidos_sup = modelos_pedidos[0]                
                modelos_pedidos_no_sup = modelos_pedidos[1]
                diccionario_permitidos_sup =  diccionario_relacion_modelos_permitidos[0]                
                diccionario_permitidos_no_sup =  diccionario_relacion_modelos_permitidos[1]
                ...
                #checking unsupervised models
                modelos_no_sup_correctos = True
                i=0
                while(modelos_no_sup_correctos and i<len(modelos_pedidos_no_sup)):
                    modelo_comprobar = modelos_pedidos_no_sup[i]
                    if modelo_comprobar not in diccionario_permitidos_no_sup:
                        modelos_no_sup_correctos = False
                        raise ValueError('Any of the unsupervised models specified for the event ' + list_of_events[indice] + ' and the learning ' + aprendizaje  + ' is not allowed')
                    i+=1
                    
                
            else:#only supervised or unsupervised learning
...
def check_lists_of_features_specified_by_user(list_of_events,list_of_lists_relevant_features_specified_by_user,list_of_lists_of_discarded_variables_by_user):
    '''
    It allows to check if the length of the list of events, relevant user features and discarded user features match
    ...
    :exception ValueError: If the number of elements in the three list does not match
...
def checking_percentil_value(percentil_value):
    ...
    Parameters:
    :param float percentil_value: Value of the percentaje used to compute the percentil taht will be used to select the number
    of features that will be discarded for the selection of relevant features in the mutual information function

    :exception ValueError: If the value of the percentil is not in the range (0,100)
    ...
    if(percentil_value < 0 or percentil_value > 100):
        raise ValueError('The minimum value for the percentil of the selected features by the mutual information function must be in the range [0,100]')            
...
def generate_information_about_current_execution(list_of_events,list_of_learnings,list_of_models,target,percentil,list_of_lists_relevant_variables_by_user,list_of_lists_discarded_variables_by_user):
    '''
    It allows to generate a formatted string with the all the information of the current execution
    
    Parameters:
    :param list list_of_events: List of the events to be processed
    :param list list_of_learnings: List with the type of the available learnings to be applied
    :param list list_of_models: List with the names of the models to be applied
    :param str target: Objective feature
    :param float percentil: Value of to cumpute the percentil of features to be selected
    :param list list_of_lists_relevant_variables_by_user: List of lists with the relevant features for each event
    :param list list_of_lists_discarded_variables_by_user: List of lists  with the discarded features for each event
    

    :return gathered_information: Information about the current process
    :rtype: str
    '''
    ...
    for index_of_event in range(len(list_of_events)):                             
        ...
        if(learning == glod.get_all_learning_modes_name()):            
            #looping supervised
            ...
            #looping unsupervised
            gathered_information+="*\t-> " + str(learnings_dictionary[2]) + "\n"            
            modelos_no_sup = models[1]
            for current_model in modelos_no_sup:
               gathered_information+="*\t\t-> " + current_model + "\n"                    
        else:
