...
from RHOASo import seleccion_nueva as rhoaso
...
def get_depth(model_name, dictionary_supervised_models, max_depth, features, target, df_catalogued_data, ruta_directorio_modelo_supervisado, formato):
    ...
    '''Computes dept or number of estimtors for the model using random search'''
    ...
    if model_name == dictionary_supervised_models[1]: #Tree
        parameter_to_compute = glod.get_model_max_depth_parameter_name()
    elif model_name == dictionary_supervised_models[2]: #Ada
        parameter_to_compute = glod.get_model_n_estimators_parameter_name()
...
def select_optimal_parameters_current_model(model_name, diccionario_modelos_sup, diccionario_modelos_no_sup, df_parameter_selection, features, target, fichero_log, ruta_directorio_modelo_supervisado, report_dict, formato, enco, max_pars, exponent):
    '''
    ...
    selected_parameters = []
    max_pars_ensemble = {glod.get_model_max_depth_parameter_name():max_depth, glod.get_model_n_estimators_parameter_name():max_depth}
    max_pars_mlp = {glod.get_model_hidden_layer_sizes():(max_depth, max_depth)}


    if model_name == diccionario_modelos_sup[1]:#Tree
        try:
            depth, ruta_imagen = get_depth(model_name, diccionario_modelos_sup, max_depth, features, target, df_parameter_selection, ruta_directorio_modelo_supervisado, glod.get_output_img_files_extension())
            selected_parameters = [int(depth)]
            report_dict[model_name][glod.get_model_parameters_plot_name()] = "'" + ruta_imagen + "'"
        except Exception as excep:
            print(excep)
            depth = 0
            selected_parameters = [depth]
            report_dict[model_name][glod.get_model_parameters_plot_name()] = "''"
        ...
        try:
            ...
            parameters, accuracy, ac, num_evals = rhoaso.selection_n(GradientBoostingClassifier, df_parameter_selection[features], df_parameter_selection[target], max_pars_ensemble, 1, exponent=exponent)
        ...
        try:
            name = 'accuracy_matrix_'+model_name
            parameters, accuracy, ac, num_evals = rhoaso.selection_n(RandomForestClassifier, df_parameter_selection[features], df_parameter_selection[target], max_pars_ensemble, 1, exponent=exponent)
            depth = parameters[glod.get_model_max_depth_parameter_name()]
            estimators = parameters[glod.get_model_n_estimators_parameter_name()]
            acc = transform_dic_to_matrix(ac, max_pars)
            resultados = plot_matrix(acc, max(max_pars)+1, name, ruta_directorio_modelo_supervisado, formato)
        ...
        try:
            name = 'accuracy_matrix_'+model_name
            parameters, accuracy, ac, num_evals = rhoaso.selection_n(MLPClassifier, df_parameter_selection[features], df_parameter_selection[target], max_pars_mlp, 1, exponent=exponent)
